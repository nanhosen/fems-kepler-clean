...
import { makeClimoDates } from '../utils/makeClimoDates.js' 
...
const ChartComponent = (data) => {
	...
	useEffect(() =>{
		...
		const climoData = data.climoData
		console.log('chartDataaaaaaaa stnData', stnData)
		console.log('all chart data ???????????????????? stnData', data)
		// console.log('this is ChartRankData shold be empty', ChartRankData)
		if(stnData && climoData){
			// console.log('bojf keys', Object.keys(stnData))
			const climoStuff = makeClimoDates(climoData)
			console.log('climoStuff', climoStuff)
			if(stnData.data && stnData.fieldOrder){
				...
				setAvgVals(climoStuff.avgForChart)
				setMaxVals(climoStuff.maxForChart)
				setMinVals(climoStuff.minForChart)
				// console.log('lengths compare set second', dates.length, dateNoDups.length)
				// setDateValArray(dates)
			}

		}
	},[data.data, data.climoData])
	...
	useEffect(() =>{
		if(chartRankData && chartEcData && avgVals && maxVals){
			console.log('chartRankData', chartRankData)
      ...
      title:{
        text: `ERC and Fire Season Percentile Rank `
      },
      subtitles: [{
        text: `Station: ${data.stn} Model: ${data.model}`
      }],
      charts: [{
        axisX: {
          lineThickness: 5,
	...
	},[chartRankData, chartEcData, avgVals, maxVals])
